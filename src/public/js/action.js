...
function comenzar_creacion() {
    ...
    if (creacion_valida) {
        ...
        consultar_api(protocolo + '://' + direccion + ':' +
            puerto + '/api/' + modelo_fisico_selecionado + '/system');
...
function cargar_posibles_sujetos_modelo(json) {
    ...
    json.forEach(element => {
        contenido_carga += '<li id="visivilidad_sujetos_para_seleccion_' + element['$']['id'] + '"><div class="form-check"><input type="checkbox" class="form-check-input sujeto_para_seleccion_padre" id="sujetos_para_seleccion_' + element['$']['id'] + '" onclick="verificar_seleccion_hijo_pagre(this);" data-puro_id="' + element['$']['id'] + '" data-puro_name="' + element['$']['name'] + '" data-oculto="false"><label class="form-check-label" for="sujetos_para_seleccion_' + element['$']['id'] + '">' + element['$']['name'] + '</label></div>';
        contenido_seleccion += '<li style="display: none;" id="visivilidad_sujetos_seleccionado_' + element['$']['id'] + '"><div class="form-check"><input type="checkbox" class="form-check-input sujeto_seleccionado_padre" id="sujetos_seleccionado_' + element['$']['id'] + '" onclick="verificar_seleccion_hijo_pagre(this);" data-puro_id="' + element['$']['id'] + '" data-puro_name="' + element['$']['name'] + '" data-oculto="true"><label class="form-check-label" for="sujetos_seleccionado_' + element['$']['id'] + '">' + element['$']['name'] + '</label></div>';
        if (element['iotSubsystem']) {
            contenido_carga += '<ul>';
            contenido_seleccion += '<ul>';
            element['iotSubsystem'].forEach(subSystem => {
                contenido_carga += '<li id="visivilidad_sujetos_para_seleccion_' + subSystem['$']['id'] + '"><div class="form-check"><input type="checkbox" class="form-check-input sujeto_para_seleccion_hijo" name="sujetos_para_seleccion_' + element['$']['id'] + '" id="sujetos_para_seleccion_' + subSystem['$']['id'] + '" onclick="verificar_seleccion_hijo_pagre(this);" data-puro_id="' + subSystem['$']['id'] + '" data-puro_name="' + subSystem['$']['name'] + '" data-oculto="false"><label class="form-check-label" for="sujetos_para_seleccion_' + subSystem['$']['id'] + '">' + subSystem['$']['name'] + '</label></div></li>';
                contenido_seleccion += '<li style="display: none;" id="visivilidad_sujetos_seleccionado_' + subSystem['$']['id'] + '"><div class="form-check"><input type="checkbox" class="form-check-input sujeto_seleccionado_hijo" name="sujetos_seleccionado_' + element['$']['id'] + '" id="sujetos_seleccionado_' + subSystem['$']['id'] + '" onclick="verificar_seleccion_hijo_pagre(this);" data-puro_id="' + subSystem['$']['id'] + '" data-puro_name="' + subSystem['$']['name'] + '" data-oculto="true"><label class="form-check-label" for="sujetos_seleccionado_' + subSystem['$']['id'] + '">' + subSystem['$']['name'] + '</label></div></li>';
            });
            contenido_carga += '</ul>';
            contenido_seleccion += '</ul>';
        }
...
function seleccionar_sujetos_modal() {
    $('#modal_seleccion_sujeto').modal('show');
}

function verificar_seleccion_hijo_pagre(elemento) {
    if (elemento.name) { // Si es hijo
...
function extraer_datos_sujeto_e_hijos_lista_check(principal, contraria) {
    var elementos = document.getElementsByClassName('sujeto_' + principal + '_padre');
        ...
        if (check.type == 'checkbox' && check.checked) {
            ...
            Array.from(hijos).forEach(x => {
                if (x.type == "checkbox" && x.checked) {
                    ...
                    document.getElementById('visivilidad_sujetos_' + principal + '_' + id_puro_hijo).style.display = 'none';
                    document.getElementById('visivilidad_sujetos_' + contraria + '_' + id_puro_hijo).style.display = 'list-item';
                    x.checked = false;
                    x.dataset.oculto = true;
                    document.getElementById('sujetos_' + contraria + '_' + id_puro_hijo).dataset.oculto = false;
                }
                ocultar_padre_principal = ocultar_padre_principal && (x.dataset.oculto == 'true');
            });
            if (ocultar_padre_principal) {
                document.getElementById('visivilidad_' + check.id).style.display = 'none';
                check.dataset.oculto = true;
            }
            check.checked = false;
            document.getElementById('sujetos_' + contraria + '_' + id_puro_padre).dataset.oculto = false;
            document.getElementById('visivilidad_sujetos_' + contraria + '_' + id_puro_padre).style.display = 'list-item';
        }
    });
}
...
function actualizar_sujetos() {
    ...
    document.getElementById('sujetos_seleccion').value = JSON.stringify(seleccion);
    $('#modal_seleccion_sujeto').modal('hide');
}

function extraer_datos_sujeto() {
    var padres = document.querySelectorAll('#lista_sujetos_seleccionados > li > div > input');
    var contenido = "";
    var new_obj = [];
        ...
        if (pad.dataset.oculto == 'false') {
            ...
            Array.from(hijos).forEach(hj => {
                if (hj.dataset.oculto == 'false') {
                    ...
                    aux_hijos += '<li data-puro_id="' + id_h + '" data-puro_name="' + name_h + '">' + name_h + '</li>';
                    var obj_hijos = {
                        id: id_h,
                        name: name_h
                    };
            ...
            contenido += '<li data-puro_id="' + id + '" data-puro_name="' + name + '">' + name + aux_hijos + '</li>';
        }
    });
    if (contenido != "") {
        contenido = '<div class="col-6 col-md-4"><hr><h4>Systema</h4><ul>' + contenido + '</ul></div>';
        document.getElementById('btn_fin_seleccion_sujetos').style.display = 'inline-block';
    } else {
        document.getElementById('btn_fin_seleccion_sujetos').style.display = 'none';
    }
    document.getElementById('lista_sujetos_seleccionados_oficial').innerHTML = contenido;
    return new_obj;
}

function finalizar_seccion_sujetos() {
    document.getElementById('btn_seleccionar_sujetos_modal').style.display = 'none';
    document.getElementById('btn_fin_seleccion_sujetos').style.display = 'none';
