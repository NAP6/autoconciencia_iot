...
class database2 {
    ...
    qwerty(sql) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            var rows, fields;
            try {
                [rows, fields] = yield connection.execute(sql);
            }
            catch (e) {
                console.log("++++++++++++++++++++++++++++++++++++++++++++");
                console.log(e);
    ...
    architecture(architecture, modelID) {
        return __awaiter(this, void 0, void 0, function* () {
            yield this.insertar_sistemas_recursivo(modelID, "NULL", architecture.modelIoTSystem);
            ...
            yield this.insertar_entidades_recursivo(architecture.modelEntity, [
                modelID,
                "NULL",
            ]);
            var dataColumn = architecture.modelDataColumn;
            for (var i = 0; i < dataColumn.length; i++) {
                var insertedColumn = yield this.qwerty(dataColumn[i].toSqlInsert(["/@/MODELO/@/"], [modelID]));
                var insertedID = insertedColumn.insertId;
                //aqui insertar las relaciones
                dataColumn[i].propertyToDataColumn.forEach((rel) => __awaiter(this, void 0, void 0, function* () {
                    var new_sql = `INSERT INTO relation_process_mapeo (ma_id, id_flow, id_proces, id_colum) values (${modelID}, ${rel[0]}, ${rel[1]}, ${insertedID})`;
                    yield this.qwerty(new_sql);
                }));
            }
        });
    }
    insertar_sistemas_recursivo(modelID, idPadre, system) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (var i = 0; i < systems.length; i++) {
                yield this.insertar_sistemas_recursivo(modelID, system.id.toString(), systems[i]);
            }
        });
    }
            ...
            for (var i = 0; i < entidades.length; i++) {
                ...
                if (entidades[i].propertys.length > 0) {
                    for (var j = 0; j < entidades[i].propertys.length; j++) {
                        ...
                        yield this.relation_propiedad_flujo(entidades[i].propertys[j].id.toString(), entidades[i].propertys[j].dataFlow, value[0], entidades[i].id.toString());
                    }
                }
                ...
                yield this.relation_sujeto_objeto(entidades[i].id.toString(), entidades[i].iotSystem, value[0]);
            }
        });
    }
    ...
    relation_propiedad_flujo(prop_id, flujos, ma_id, obj_id) {
        return __awaiter(this, void 0, void 0, function* () {
            var connection = yield this.conectar();
            flujos.forEach((flu) => {
                connection.execute(`insert INTO propiedad_flujodatos (pro_id, flu_id, ma_id, obj_id) VALUES (${prop_id},${flu.id}, ${ma_id}, ${obj_id})`);
            });
            connection.end();
        });
    }
